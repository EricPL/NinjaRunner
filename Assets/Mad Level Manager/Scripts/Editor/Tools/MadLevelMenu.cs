...
public class MadLevelMenu : MonoBehaviour {
    ...
    [MenuItem("Tools/Mad Level Manager/Sounds/Add Play Clip", false, 160)]
    static void AddPlayClip() {
        var go = Selection.activeGameObject;
        if (!CheckPlayClipValidObject(go)) {
            return;
        }

        go.AddComponent<MadPlayAudioClip>();
    }

    static bool CheckPlayClipValidObject(GameObject go) {
        if (go == null) {
            EditorUtility.DisplayDialog("Cannot Add Play Clip", "Please select object with MadSprite or MadLevelIcon component attached.", "OK");
            return false;
        }
    ...
    [MenuItem("Tools/Mad Level Manager/Level Generator", false, 903)]
    static void LevelGenerator() {
        MadLevelGeneratorTool.OpenWindow();
    }
    
    [MenuItem("Tools/Mad Level Manager/Select Active Configuration %&b", false, 904)]
...
//            EditorGUIUtility.PingObject(active);
    ...
    [MenuItem("Tools/Mad Level Manager/Settings", false, 904)]
